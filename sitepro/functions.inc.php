...
function checkSiteRedirects(SiteInfo $siteInfo, SiteRequestInfo $requestInfo, array $redirectItems) {
	...
	foreach ($redirectItems as $item) {
		...
		if (!$item['exact'] && $fromUrl === '' && $requestUri !== '') {
			$redirectTo = getBaseUrl();
			header('Location: '. $redirectTo, true, 301);
			exit();
		}
		else if (!$item['exact'] && $fromUrl === '' && $requestUri === '' ) {
			return;
		}
		else if (($item['exact'] && $fromUrl == $requestUri) || (!$item['exact'] && strpos($requestUri . '/', $fromUrl . '/' ) === 0) || ($requestUri == '' && $requestUri == $fromUrl)) {
			if ($toUrl['type'] == 'page') {
				$defLang = ($siteInfo->defLang ? $siteInfo->defLang : null);
	...
	if (trim(getBaseUrl() . $fromUrl, '/') === trim($redirectTo, '/') || trim(getBaseUrl() . $fromUrl, '/') === trim(getBaseUrl() . $redirectTo, '/')) {
		return;
	}
